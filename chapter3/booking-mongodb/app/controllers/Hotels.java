...
public class Hotels extends Application {
    ...
    public static void confirmBooking(String id, Booking booking) {
        Hotel hotel = Hotel.findById(id);
        booking.hotel = hotel;
        ...
        if(params.get("confirm") != null) {
            booking.save();
            flash.success("Thank you, %s, your confimation number for %s is %s", connected().name, hotel.name, booking.getId());
            index();
        }
    ...
    public static void cancelBooking(String id) {
        ...
        booking.delete();
        flash.success("Booking cancelled for confirmation number %s", booking.getId());
        index();
    }
